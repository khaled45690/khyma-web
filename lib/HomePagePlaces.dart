...
class HomePagePlaces extends StatefulWidget {
  ...
  final String dbUrl;
  final String title;
  HomePagePlaces({Key key, this.dbUrl, this.title}) : super(key: key);
  _HomePagePlacesState createState() => _HomePagePlacesState();
}
...
class _HomePagePlacesState extends State<HomePagePlaces> {
  ...
  List<meal> myTempSortedHalawany = <meal>[];
  List<meal> myTempSortedCafe = <meal>[];
  List<meal> myTempSortedDrinks = <meal>[];
  List<meal> myTempSortedAll = <meal>[];
  List<meal> currentList = <meal>[];
  List<String> dropDownValues = ["الكل", "حلواني", "مطعم/كافيه", "خيمة"];
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          //         onPressed: () {
          //           setState(() {
          //             mealsList = myTempSortedCafe;
          //             currentList = myTempSortedCafe;
          //             myController.text = "";
          //           });
          //         },
          //         child: Text("وجبات"),
          ...
          //         onPressed: () {
          //           setState(() {
          //             mealsList = myTempSortedHalawany;
          //             currentList = myTempSortedHalawany;
          //             myController.text = "";
          //           });
          //         },
          //         child: Text("حلويات"),
  ...
  onChange(value) {
    setState(() {
      ...
      if(value == "الكل"){
        ...
        currentList = myTempSortedAll;
        myController.text = "";
      }else if(value == "حلواني"){
        mealsList = myTempSortedHalawany;
        currentList = myTempSortedHalawany;
        myController.text = "";
      }else if(value == "مطعم/كافيه"){
        mealsList = myTempSortedCafe;
        currentList = myTempSortedCafe;
        myController.text = "";
      }else if(value == "خيمة"){
  ...
  _fetchDogsBreed(String url) async {
    ...
    if (response.statusCode == 200) {
      ...
      jsonResponse["message"].forEach((element) {
        ...
        tempMeal.catigory = element["type"];
      ...
      myTempSortedHalawany = myTempMeals
          .where((element) => element.catigory.toString().contains("حلوانى"))
          .toList();
      myTempSortedCafe = myTempMeals
          .where((element) => element.catigory.toString().contains("مطعم و كافيه"))
          .toList();
      myTempSortedDrinks = myTempMeals
          .where((element) => element.catigory.toString().contains("الخيمة"))
          .toList();
